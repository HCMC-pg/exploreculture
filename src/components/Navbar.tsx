    ...
    >
      <div className="max-w-7xl mx-auto px-2.5 sm:px-6 py-2 flex items-center justify-between gap-1.5 sm:gap-2">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          {/* Gmail Required Login Button / Sync Status */}
          {(!user.isGoogleLinked || !user.googleEmail) ? (
            <button
              ...
              id="navbar-login-gmail-btn"
              title="Yêu cầu đăng nhập Gmail để lưu tiến trình học tập của bạn"
              className="px-2.5 sm:px-3 py-1.5 rounded-xl bg-gradient-to-r from-red-600/90 via-amber-500 to-yellow-500 hover:from-red-500 hover:to-amber-400 text-stone-950 text-xs font-black flex items-center gap-1.5 shadow-lg shadow-amber-500/20 transition-all hover:scale-105 animate-pulse shrink-0"
            >
              ...
              <span className="hidden sm:inline">Đăng Nhập Gmail Để Lưu Tiến Trình</span>
              <span className="sm:hidden">Lưu Gmail</span>
            </button>
          ) : (
            <div className="hidden lg:flex items-center gap-1.5 shrink-0">
              <div 
                onClick={onOpenProfile}
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-blue-950/40 border border-blue-500/40 text-[11px] text-blue-300 font-medium cursor-pointer hover:bg-blue-900/40 transition-colors"
                title={`Tiến trình học tập đã lưu với Gmail: ${user.googleEmail} • Đồng bộ lần cuối: ${lastSyncedTime}`}
              >
                <span className={`w-2 h-2 rounded-full ${isCloudSyncing ? 'bg-amber-400 animate-spin' : 'bg-emerald-400 animate-pulse'}`} />
                <span className="truncate max-w-[120px] font-mono text-[10px]">{user.googleEmail}</span>
                <span className="px-1.5 py-0.2 rounded bg-blue-500/20 text-blue-300 text-[9px] font-bold">
                  {isCloudSyncing ? 'Đang lưu...' : 'Đã lưu'}
                </span>
              </div>
              {onManualSync && (
                <button
                  ...
                  title="Nhấn để đồng bộ dữ liệu lên máy chủ ngay lập tức"
                  className="p-1.5 rounded-xl bg-stone-900 hover:bg-stone-800 text-stone-300 hover:text-emerald-400 border border-stone-800 transition-colors shadow-sm"
                >
