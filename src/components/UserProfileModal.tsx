...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  const [gmailWarningNotice, setGmailWarningNotice] = useState(false);

  const handleSaveAllPreferences = async () => {
    if (!isGmailLoggedIn) {
      sound.playError();
      setGmailWarningNotice(true);
      if (onOpenAuthModal) {
        onOpenAuthModal();
      }
      return;
          ...
          <div className="pt-1 flex items-center justify-center gap-2 flex-wrap">
            ...
            {userEmail && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 text-[11px] text-emerald-300 font-bold transition-all hover:scale-105"
                title="Nhấn để đồng bộ dữ liệu với máy chủ ngay"
              >
                <Cloud className={`w-3.5 h-3.5 ${isSavingPrefs ? 'animate-bounce text-amber-400' : 'text-emerald-400'}`} />
                <span>{isSavingPrefs ? 'Đang lưu...' : 'Đồng Bộ Đám Mây'}</span>
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-1">
          ...
          {activeTab === 'habits' && (
            <div className="space-y-5 animate-fadeIn">
              ...
              {/* Gmail Authentication Status Banner */}
              {!isGmailLoggedIn ? (
                <div className="p-3.5 rounded-2xl bg-gradient-to-r from-red-950/60 via-amber-950/40 to-stone-950 border border-amber-500/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-md">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-bold text-amber-200">Yêu cầu đăng nhập Gmail để lưu tiến trình học tập</p>
                      <p className="text-[11px] text-stone-300">
                        Thói quen, sở thích và chuỗi ngày học của bạn cần được bảo lưu theo Gmail cá nhân để đồng bộ xuyên suốt.
                      </p>
                    </div>
                  </div>
                  {onOpenAuthModal && (
                ...
                <div className="p-3 rounded-2xl bg-emerald-950/30 border border-emerald-500/40 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse shrink-0"></span>
                    <p className="text-xs text-stone-200">
                      <strong className="text-emerald-300">Đã Liên Kết Gmail: </strong>
                      <span className="font-mono text-amber-300">{user.googleEmail || user.email}</span> (Tiến trình & sở thích đang được lưu trữ an toàn)
                    </p>
                  </div>
