  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-fadeIn">
      <div className="w-full max-w-2xl bg-stone-900 border-2 border-amber-500/40 rounded-3xl p-5 sm:p-6 shadow-2xl text-stone-100 space-y-4 relative overflow-hidden max-h-[92vh] flex flex-col">
        ...
        {/* Gmail Login Status & Persistence Banner */}
        <div className="p-3.5 rounded-2xl border transition-all shrink-0 bg-stone-950/90 shadow-lg">
          {user.googleEmail && user.googleEmail.endsWith('@gmail.com') ? (
              ...
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs font-bold text-emerald-300">Đã Liên Kết Gmail Chính Chủ</span>
                    <span className="px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-400 text-[9px] font-bold">Lưu Tự Động</span>
                  </div>
                  <p className="text-[11px] font-mono text-stone-300 truncate">{user.googleEmail}</p>
                </div>
              </div>

              {onOpenAuthModal && (
                <button
                  onClick={() => { sound.playClick(); onOpenAuthModal(); }}
                  className="px-3 py-1.5 rounded-xl bg-stone-900 hover:bg-stone-800 border border-stone-700 text-stone-300 text-xs font-semibold shrink-0"
                >
                  Quản Lý / Đổi
                </button>
              )}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-gradient-to-r from-red-950/40 via-amber-950/30 to-stone-950 p-2.5 rounded-xl border border-amber-500/40">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="text-xs font-bold text-amber-300 flex items-center gap-1.5">
                    <span>Yêu Cầu Đăng Nhập Gmail Để Lưu Tiến Trình</span>
                  </h4>
                  <p className="text-[11px] text-stone-300 leading-tight mt-0.5">
                    Đăng nhập Gmail cá nhân (@gmail.com) để bảo lưu vĩnh viễn kết quả giải đố, huy hiệu và điểm LP.
                  </p>
                </div>
              </div>
              ...
              {onOpenAuthModal && (
                <button
                  onClick={() => { sound.playClick(); onOpenAuthModal(); }}
                  className="w-full sm:w-auto px-4 py-2 rounded-xl bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-500 hover:to-amber-500 text-white text-xs font-extrabold flex items-center justify-center gap-1.5 shadow-md shadow-red-600/30 shrink-0 transition-transform active:scale-95"
                >
